...
func (r *Repo) GetByID(ctx context.Context, id int64) (*Post, error) {
	q := fmt.Sprintf(`
		SELECT 
			p.id, p.author_id, u.username, p.category_id, c.name, p.title, p.slug, p.markdown_content, p.html_content, p.created_at FROM %s AS p
...
func (r *Repo) GetBySlug(ctx context.Context, slug string) (*Post, error) {
	...
	err := r.db.QueryRowContext(ctx, q, slug).Scan(
		&p.ID, &p.AuthorID, &p.AuthorName, &p.CategoryID, &p.CategoryName, &p.Title, &p.Slug, &p.MarkdownContent, &p.HTMLContent, &p.CreatedAt,
	)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrPostNotFound
	}
