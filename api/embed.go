...
// /openapi.yaml, /openapi.json, and /docs, and by internal/api/router's drift tests.
package api
...
// Audiences match auth.Claims.Role for the four DB-seeded roles, plus "anonymous" for unauthenticated callers.
var Audiences = []string{"anonymous", "Subscriber", "Author", "Editor", "Admin"}

// SpecJSON is the full spec as JSON. Used by the existing drift test that asserts every chi route is documented somewhere.
var SpecJSON []byte

// SpecYAMLByRole / SpecJSONByRole hold one pre-filtered copy of the spec per audience.
// The /openapi.{yaml,json} handler picks the right slice based on the caller's bearer token.
var (
...
func init() {
	...
	if err != nil {
		// A malformed embedded spec should fail loud at process startup, not at first request.
